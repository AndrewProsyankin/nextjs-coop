...
export default function ProductPage() {
  ...
  useEffect(() => {
    setIsMounted(true); // Устанавливаем флаг после завершения гидрации
  }, []);
              ...
              typedProductsData.map((product: Product) => {
                ...
                return (
                  <div key={product.id} className="group">
                    ...
                    {/* Проверяем, завершена ли гидрация */}
                    {isMounted && (
                      <div>
                        <button
                          onClick={() => addToCart(product)}
                          className={`mt-4 w-full rounded-md px-4 py-2 text-sm font-medium text-white ${
                            isInCart ? 'bg-green-500 cursor-default' : 'bg-[#98730C] hover:bg-[#f0bd7a]'
                          }`}
                          disabled={isInCart}
                        >
